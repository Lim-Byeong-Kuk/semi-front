...
const APITestPage = () => {
  ...
  const addOneProduct = async () => {
    ...
    console.log(data);
    const product = await LocalStorageService.saveByOne(
      storageEnum.Class.Product,
      data
    );
  ...
  const initProductData = async () => {
    const product = await LocalStorageService.initData(
      storageEnum.Class.Product,
      phonecaseProducts
    );
    console.log(product);
  ...
  const findAllData = async (className) => {
    ...
    if (!setState) return storageEnum.Result.Failure;
  ...
  return (
    <div>
      <NavBar />
      <button onClick={addProducts}>phonecaseProducts를 추가</button>
      <br />
      <button onClick={addOneProduct}>랜덤으로 Product 한 개 추가</button>
      <br />
      <button onClick={resetProductData}>Product Data 모두 삭제</button>
      <br />
      <button onClick={initProductData}>Product Data 초기화</button>
      <br />
      <button onClick={() => findAllData(storageEnum.Class.Product)}>
        Product 데이터 로드 및 세팅
      </button>
      <br />
      <input
      ...
      />
      <button onClick={() => findOneData(storageEnum.Class.Product, idx)}>
        Product 원하는 데이터 불러오기
      </button>
      <hr />
      {products &&
              ...
              {editProducts && editProducts.length > 0 && (
                <div>
                  ...
                  <input
                    type="text"
                    name="productId"
                    value={editProducts[idx].productId}
                  />
                  name :{" "}
                  <input
                    type="text"
                    name="name"
                    value={editProducts[idx].name}
                    onChange={(e) => {
                  ...
                  <button
                    onClick={() =>
                      deleteOneData(storageEnum.Class.Product, item.productId)
                    }
                  >
                    삭제하기
                  </button>
